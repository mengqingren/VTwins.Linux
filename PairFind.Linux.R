...
option_list = list(
  make_option(c("-d", "--dataset"), type="character", default=NULL,
              help="relative abundance matrix file name", metavar="character"),
  make_option(c("-g", "--group"), type="character", default=NULL,
              help="group phenotype information file name", metavar="character"),
  ...
  make_option(c("-n", "--downpercent"), type="double", default=0.2, 
              help="down percent of shuffle pair [default %default]",metavar="number"),
  make_option(c("-a", "--savepath"), type="character", default='./', 
              help="where to store the output", metavar="character"),
  make_option(c("-w", "--ShuffleWstat"), type="character", default="ShuffleWstat", 
              help="output shuffle W state file name [default %default]", metavar="character"),
  make_option(c("-b", "--BoundarySample"), type="character", default="BoundarySample", 
              help="where to store the output [default %default]", metavar="character"),
  make_option(c("-p", "--BoundaryPair"), type="character", default="BoundaryPair", 
              help="where to store the output [default %default]", metavar="character"),
  make_option(c("-o", "--output"), type="character", default="Results.txt",
              help="where to store the output [default %default]", metavar="character"),
  make_option(c("-v", "--verbose"), action="store_true", default=TRUE,
              help="It will need some time to run for VTwins version 1"),
  make_option(c("-c", "--PvalueCutoff"), type="double", default=0.05,
              help="raw p value cutoff [default %default]",metavar="number")
...
if (is.null(opt$dataset) | is.null(opt$group) | is.null(opt$output)) {
  print_help(opt_parser)
  stop("matrix file and group information file and output file name must be supplied", call.=FALSE)
}

if (is.integer(opt$shuffletime) | is.double(opt$uppercent) | is.double(opt$downpercent)) {
  if (!((opt$uppercent <= 1 & opt$uppercent >= 0) & (opt$downpercent <= 1 & opt$downpercent >= 0) & (opt$uppercent >= opt$uppercent))) {
    stop("You must input the right shuffle time, uppercent and downpercent", call.=FALSE)
  }
}

if (! opt$method %in% c("euclidean","maximum","manhattan", "canberra", "binary","minkowski")) {
...
pair_find<-function(data=data,phenodata=data.frame(),k="euclidean",SavePath = NULL,ShuffleWstat = NULL,PvalueCutoff=0.05, BoundarySample = NULL,BoundaryPair=NULL,ShuffleTime=10000,DownPercent = 0.2,Uppercent=0.8){ # colnames(phenodata) = c("id","grp"); grp1 = "Ctrl", grp2 = "Disease"
  ...
  #RAN<- data %>% dplyr::filter(grp=="grp1") #data[1:RAN_num,]
  #RAP<- data %>% dplyr::filter(grp=="grp2") #data[(RAN_num+1):(RAN_num+RAP_num),]
